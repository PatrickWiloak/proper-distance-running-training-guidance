# Training Periodization Visual Guide

## The Big Picture: Training Phases Over Time

```
MACROCYCLE (6-12 Months)
│
├─ Phase 1: BASE (12-16 weeks)
│  │ Volume: ▲▲▲ Increasing
│  │ Intensity: ▼▼ Low
│  └─ Goal: Build aerobic foundation
│
├─ Phase 2: BUILD (8-12 weeks)
│  │ Volume: ▲ Maintain/Slight increase
│  │ Intensity: ▲▲ Add tempo work
│  └─ Goal: Develop lactate threshold
│
├─ Phase 3: PEAK (6-8 weeks)
│  │ Volume: ═ Maintain
│  │ Intensity: ▲▲▲ High (tempo + intervals)
│  └─ Goal: Race-specific fitness
│
├─ Phase 4: TAPER (1-3 weeks)
│  │ Volume: ▼▼▼ Decrease 30-50%
│  │ Intensity: ▲ Some intensity to stay sharp
│  └─ Goal: Fresh for race
│
└─ Phase 5: RECOVERY (2-4 weeks)
   │ Volume: ▼▼ Low
   │ Intensity: ▼▼▼ None
   └─ Goal: Physical & mental reset
```

## Phase Breakdown by Percentage

### Weekly Mileage Distribution

| Phase | Easy (%) | Tempo (%) | Intervals (%) | Long Run (%) |
|-------|----------|-----------|---------------|--------------|
| **Base** | 80-85% | 0-5% | 0% | 15-20% |
| **Build** | 70-75% | 10-15% | 0-5% | 15-20% |
| **Peak** | 65-70% | 10-15% | 5-10% | 15-20% |
| **Taper** | 70-75% | 5-10% | 5-10% | 10-15% |
| **Recovery** | 95-100% | 0% | 0% | 5% |

## Timeline Examples by Goal Race

### 5K to 10K Training

```
Month 1-3:  BASE ████████████ (12 weeks)
            └─ Easy runs, build to 30-40 mpw

Month 4-5:  BUILD ████████ (8 weeks)
            └─ Add tempo, maintain volume

Month 6-7:  PEAK ██████ (6 weeks)
            └─ Tempo + intervals, race-specific

Week 25:    TAPER █ (1 week)
            └─ Reduce volume 40%

Week 26:    RACE DAY! 🏁

Weeks 27-30: RECOVERY ████ (4 weeks)
            └─ Easy running, reset
```

### Half Marathon Training

```
Month 1-4:  BASE ████████████████ (16 weeks)
            └─ Build to 35-45 mpw gradually

Month 5-6:  BUILD ████████ (8 weeks)
            └─ Weekly tempo runs, progressive long runs

Month 7-8:  PEAK ████████ (8 weeks)
            └─ Half marathon pace practice

Week 33-34: TAPER ██ (2 weeks)
            └─ Reduce volume 30-40%

Week 35:    RACE DAY! 🏁

Weeks 36-39: RECOVERY ████ (4 weeks)
```

### Marathon Training

```
Month 1-5:  BASE ████████████████████ (20 weeks)
            └─ Build to 40-60 mpw, long runs to 18 miles

Month 6-8:  BUILD ████████████ (12 weeks)
            └─ Tempo runs, marathon pace practice

Month 9-10: PEAK ██████████ (10 weeks)
            └─ Long runs with MP segments, peak mileage

Week 41-43: TAPER ███ (3 weeks)
            └─ Gradual volume reduction

Week 44:    RACE DAY! 🏁

Weeks 45-48: RECOVERY ████ (4 weeks)
```

## Weekly Microcycle Structure

### Base Phase Week
```
Mon: Easy ●●●●
Tue: Easy ●●●●
Wed: Easy ●●●
Thu: Easy ●●●●
Fri: Rest/Easy ●●
Sat: Long Run ●●●●●●●
Sun: Rest

Total: 80% easy, 20% long run stress
```

### Build Phase Week
```
Mon: Easy ●●●●
Tue: TEMPO ●●●●● (Zone 4)
Wed: Easy ●●●
Thu: Easy ●●●●
Fri: Easy ●●
Sat: Long Run ●●●●●●●
Sun: Rest

Total: 70% easy, 15% tempo, 15% long run
```

### Peak Phase Week
```
Mon: Easy ●●●●
Tue: INTERVALS ●●●●● (Zone 5)
Wed: Easy ●●●
Thu: Easy ●●●●
Fri: TEMPO ●●●●● (Zone 4)
Sat: Long Run w/ MP ●●●●●●●
Sun: Easy ●●●

Total: 65% easy, 15% threshold, 10% intervals, 10% long run
```

## Training Load Visualization

```
Training Load Over 24-Week Cycle:

Volume   │                    ╱────╲
(miles)  │                ╱───      ╲
         │            ╱───            ╲
         │        ╱───                  ╲__
         │    ╱───                           ╲
         │╱───                                 ╲___
         └────────────────────────────────────────────
          Base    Build    Peak    Taper  Race  Recovery

Intensity│                      ╱─╲
(%)      │                  ╱───   ╲
         │              ╱───         ╲
         │          ╱───               ╲
         │      ╱───                     ╲
         │  ╱───                           ╲___
         └────────────────────────────────────────────
          Base    Build    Peak    Taper  Race  Recovery
```

## The 3-to-1 Week Progression

Most effective progression: 3 weeks build, 1 week recovery

```
Week 1: Build    ████████ (40 miles)
Week 2: Build    ██████████ (45 miles)
Week 3: Build    ███████████ (48 miles)
Week 4: RECOVERY ██████ (32 miles) ← Down week
Week 5: Build    ██████████ (45 miles)
Week 6: Build    ███████████ (50 miles)
Week 7: Build    ████████████ (53 miles)
Week 8: RECOVERY ███████ (38 miles) ← Down week
```

**Why this works:**
- Allows adaptation from previous 3 weeks
- Prevents cumulative fatigue
- Reduces injury risk
- Mental break from building

## Quick Decision Guide

### "What phase am I in?"

**No goal race yet?** → BASE phase
- Build aerobic foundation
- Increase mileage gradually
- 80%+ easy running

**12-20 weeks until race?** → BUILD phase
- Add tempo runs (1x/week)
- Maintain volume
- Build lactate threshold

**6-10 weeks until race?** → PEAK phase
- Add intervals (every 7-14 days)
- Race-specific work
- 2 quality sessions/week

**1-3 weeks until race?** → TAPER phase
- Cut volume 30-50%
- Maintain some intensity
- Prioritize rest

**Just raced?** → RECOVERY phase
- Easy running only
- 2-4 weeks off hard training
- Let body repair

## Common Periodization Mistakes

❌ **Skipping base phase** → Jumping to hard workouts without foundation
✅ **Build base first** → 12-16 weeks minimum

❌ **No recovery weeks** → Building indefinitely without down weeks
✅ **3 build, 1 recovery** → Every 4th week reduce volume 20-30%

❌ **Constant intensity** → Always doing hard workouts
✅ **Progressive phases** → Build intensity gradually over months

❌ **Inadequate taper** → Training hard until race week
✅ **Proper taper** → Reduce volume 30-50% for 1-3 weeks

❌ **No off-season** → Racing and training hard year-round
✅ **Planned recovery** → 2-4 weeks easy after goal races

## Key Principles

1. **Volume before intensity** → Build mileage base before adding hard workouts
2. **Progressive overload** → Gradually increase stress over time
3. **Recovery weeks essential** → Down weeks allow adaptation
4. **Race-specific training** → Peak phase matches race demands
5. **Taper = trust your training** → Less is more in final weeks

---

**Remember:** Periodization isn't just a fancy plan - it's how you systematically build fitness while avoiding injury and overtraining. Every elite runner uses periodization. You should too.
